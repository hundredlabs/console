...
class PgAlertRepoImpl(blockingEC: ExecutionContext) extends AlertRepository with AlertRuleFmt {
  ...
  override def upsert(jobId: Long, name: String, rules: String): Future[Long] = Future {
    blocking {
      ...
      DB localTx { implicit session =>
        ...
        jsonObj.setValue(rules)
        val rulesJson = ParameterBinder(
          value = rules,
        ...
        sql"""INSERT INTO alert_spark_metrics_rules(alert_id, job_id, name, alert_rule, create_ts)
              VALUES(${alertId}, ${jobId}, ${name}, ${rulesJson}, ${ZonedDateTime.now()})
             ON CONFLICT (job_id, name)
            DO
            UPDATE SET
             alert_rule = ${rulesJson}
           """
          .update()
          .apply()
        val persistedId =
