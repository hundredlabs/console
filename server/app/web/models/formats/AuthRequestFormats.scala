...
import web.models.{ActionForbidden, AlphaRequests, AlphaSignedUpUser, BetaRequestListResponse, BetaSignedUpUser, JobWithUsage, MemberInfoResponse, OrgDetail, OrgResponse, OrgWithKeys, SignInResponse, SignOutResponse, SignupResponse, UserNotAuthenticated, UserView, WorkspaceAPIKey, WorkspaceResponse, WorkspaceViewResponse}
...
trait AuthResponseFormats {
  ...
  implicit val betaSignupsRespFmt = Json.format[BetaRequestListResponse]
  implicit val alphaSignedUpUser = Json.format[AlphaSignedUpUser]
  implicit val alphaRequestFmt = Json.format[AlphaRequests]
  ...
  implicit val jobWithUsageFmt = Json.format[JobWithUsage]
}
